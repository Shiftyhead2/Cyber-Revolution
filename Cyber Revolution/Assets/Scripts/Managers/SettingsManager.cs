...
public class SettingsManager : MonoBehaviour {
	...
	void Start(){
		...
		if (File.Exists (Application.persistentDataPath + "/gameSettings.json") == true) {
			...
			vSyncDropdown.GetComponent<Dropdown> ().value  = gameSettings.vSync = QualitySettings.vSyncCount;
			antialiasingDropdown.GetComponent<Dropdown> ().value = gameSettings.antiAliasing = QualitySettings.antiAliasing;
			textureQualityDropdown.GetComponent<Dropdown> ().value = gameSettings.textureQuality = QualitySettings.masterTextureLimit;
			fullscreenToggle.GetComponent<Toggle> ().isOn = gameSettings.fullScreen = Screen.fullScreen;
			int CurrentResolution;
			for (int i = 0; i < resolutions.Length; i++) {
				if (resolutions [i].width == Screen.currentResolution.width && resolutions [i].height == Screen.currentResolution.height) {
					CurrentResolution = i;
					resolutionDropdown.GetComponent<Dropdown> ().value = CurrentResolution;
	...
	public void LoadSettings(){
		...
		antialiasingDropdown.GetComponent<Dropdown>().value = QualitySettings.antiAliasing = gameSettings.antiAliasing;
		vSyncDropdown.GetComponent<Dropdown>().value = QualitySettings.vSyncCount = gameSettings.vSync;
		textureQualityDropdown.GetComponent<Dropdown>().value = QualitySettings.masterTextureLimit = gameSettings.textureQuality;
